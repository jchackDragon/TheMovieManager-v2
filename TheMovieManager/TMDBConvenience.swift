...
extension TMDBClient {
    ...
    */
    ...
    func getWatchlistMovies(_ completionHandlerForWatchlist: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        ...
        mutableMethod = substituteKeyInMethod(mutableMethod, key: TMDBClient.JSONResponseKeys.UserID, value: String(TMDBClient.sharedInstance().userID!))!
        /* 2. Make the request */
        let _ = self.taskForGETMethod(mutableMethod, parameters: parameters as [String:AnyObject]){(result, error) in
            
            if let error = error{
                completionHandlerForWatchlist(nil, error)
            }else{
                if let movieResult = result?[TMDBClient.JSONResponseKeys.MovieResults] as? [[String:AnyObject]]{
                    let movies = TMDBMovie.moviesFromResults(movieResult)
                    completionHandlerForWatchlist(movies,nil)
                }else{
                    completionHandlerForWatchlist(nil, NSError(domain:"getWatchlistMovies parsing", code:0, userInfo:[NSLocalizedDescriptionKey:"Could no parse getWatchlistMovies "]))
                }
            }
    ...
    func postToWatchlist(_ movie: TMDBMovie, watchlist: Bool, completionHandlerForWatchlist: @escaping (_ result: Int?, _ error: NSError?) -> Void) {
        ...
        //JsonBodyParameter
        let mutableParameter = [JSONBodyKeys.MediaType: Constants.Movie,
                                JSONBodyKeys.MediaID: movie.id,
                                JSONBodyKeys.Watchlist: watchlist] as [String : Any]
        //JSON body from body parameters
        let jsonBody = self.toJsonString(mutableParameter as [String : AnyObject])!
        //Call method
        var mutableMethod = Methods.AccountIDWatchlist
        mutableMethod = substituteKeyInMethod(mutableMethod, key: URLKeys.UserID, value: String(TMDBClient.sharedInstance().userID!))!
        
        /* 2. Make the request */
        let _ = self.taskForPOSTMethod(mutableMethod, parameters: parameters as [String:AnyObject], jsonBody: jsonBody) { (result, error) in
            
            if let error = error{
                completionHandlerForWatchlist(nil, error)
            }else{
                if let stat = result?[TMDBClient.JSONResponseKeys.StatusCode] as? Int{
                    completionHandlerForWatchlist(stat,nil)
                }else{
                    completionHandlerForWatchlist(nil, NSError(domain:"postToWatchlist parsing", code: 1, userInfo:[NSLocalizedDescriptionKey:"Could not parse postToWatchlist"]))
                }
            }
        }
        /* 3. Send the desired value(s) to completion handler */
